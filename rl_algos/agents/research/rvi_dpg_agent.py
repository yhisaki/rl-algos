...
class RVIDPGAgent(AttributeSavingMixin, AgentBase):
    saved_attributes = (
        "q",
        "q_target",
        "q_optimizer",
    ...
    def __init__(
        ...
        logger: logging.Logger = logger.getChild("RVIDPGAgent"),
        ...
        self.offset_update_tau = offset_update_tau
        self.offset = 0.0
    ...
    def _update_critic(self, batch: TrainingBatch):
        q_loss, offset_target = self.compute_q_loss_and_offset_target(batch)
        ...
        # update offset
        self.offset = (
            self.offset_update_tau * offset_target + (1 - self.offset_update_tau) * self.offset
        )
    ...
    def compute_q_loss_and_offset_target(self, batch: TrainingBatch):
        with torch.no_grad(), evaluating(self.policy_target, self.q_target):
            ...
            offset_target = torch.mean(next_q)

            q_target = batch.reward - self.offset + torch.flatten(next_q)
        ...
        if self.stats is not None:
            self.stats("offset").append(self.offset)
            self.stats("offset_target").append(offset_target.detach().cpu().float())
        ...
        return loss, offset_target.detach().cpu().float()
